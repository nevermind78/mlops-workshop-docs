...
### Instrumentation de l’API FastAPI (`app/main.py`)
...
```
### Génération de Données avec Drift {#sec-module6-driftgen}

En environnement réel, les données de production évoluent progressivement
(changement de comportement client, contexte économique, nouvelles offres, etc.).

Afin de **tester et valider le système de monitoring**, nous utilisons un script de
génération artificielle de données de production avec drift contrôlé :
`drift_data_gen.py`.

Ce script permet de :
- simuler différents niveaux de drift (faible, moyen, fort)
- créer un jeu de données de production réaliste
- tester la robustesse du système de détection
...
Ce fichier `production_data.csv` est ensuite utilisé par l’endpoint :

```http
POST /drift/check
```

pour comparer les données de référence et de production.
...
| `drift_detect.py` | Détection statistique |
| Application Insights | Monitoring & historisation |
| Alertes Azure | Décision opérationnelle |
